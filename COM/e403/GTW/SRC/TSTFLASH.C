...
int wr (void)
{
	...
	er = 0;
	i=0;
	pt_ram = (unsigned int  *)(0x000000);
	/*** ciclo di srittura e verifica su tutti gli indirizzi ***/
	for ( pt_flash = sctstr; pt_flash <=(unsigned int  *)0x4FFFFE; pt_flash++){
	/***    procedura di start write     ***/
		...
		*pt_flash = dt;
		...
		if (pt_ram == (unsigned int  *)(0x00EFFE))
		{
			pt_ram = (unsigned int  *)(0x00FFFE);
			pt_flash = (unsigned int  *)(0x40FFFE);
		}
		pt_ram++;
	}
	printf("\r\n");
...
/* Valore ritornato:   codice errore                      */
/**********************************************************/
int chip_erasesect(unsigned int  * addr)
{
...
void tstflash (void )
{
	...
	unsigned int  * sectstart;
	unsigned int  * sectend;
	unsigned long app;
	char c;
