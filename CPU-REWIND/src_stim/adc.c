...
void test_adc(void)
{
	...
	printf("\nPosizionare lo switch al centro e verificare che i led S1 e S2 siano spenti\n");
    ...
    for(i=0; i<8; i++) //in0..in7
    {
		printf("\nTest IN%bd\n",i);
		if(i == 0) 
			printf("Posizionare lo switch su NOISE1 e verificare cke il led S1 sia acceso (test IN%bd)\n",i);
		else if(i == 1) 
			printf("Posizionare lo switch su NOISE1 (test IN%bd)\n",i);
		else if(i == 2) 
			printf("Posizionare lo switch su NOISE2 e verificare cke il led S2 sia acceso (test IN%bd)\n",i);
		printf("Premere INVIO per proseguire \n");while ( (c = getchar())!= 0x0A) ;
